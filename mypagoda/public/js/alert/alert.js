const New = {
    ...
    alert: function ({ status, title, content, confirmbtn = true }) {
        ...
        if (status == "" || status == "success") {
            ...
            document.querySelector(".alert_details > h2").style.color =
                "#1FB397";
...
function show_Err_alert() {
    New.alert({
        ...
        content:
            "A server side error, try again later, or contact site support",
    });
}
...
function alert_delete(
    event,
    dataTitle = "វត្តជលធារាសម្បត្តិ",
    dataContent = "តើអ្នកជាចង់លុបទិន្នន័យនេះមែនទេ?"
) {
    New.alert({
        status: "info",
        title: dataTitle,
...
function show_infor_alert(yourContent) {
    New.alert({
        status: "info",
        title: "welcome",
        content: yourContent,
